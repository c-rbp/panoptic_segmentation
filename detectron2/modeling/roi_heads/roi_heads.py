...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            # Fix for RNNs
            start_test_score_thresh = self.test_score_thresh
            reprocess = True
            while reprocess:
                pred_instances, _ = outputs.inference(
                    start_test_score_thresh, self.test_nms_thresh, self.test_detections_per_img
                )
                if len(pred_instances[0].scores):
                    reprocess = False
                start_test_score_thresh -= 0.005
            return pred_instances
    ...
    def _forward_mask(
        ...
        if self.training:
            ...
            if isinstance(mask_logits, dict):
                vj_penalty = mask_logits['vj_penalty']
                mask_logits = mask_logits['preds']
                return {"loss_mask": mask_rcnn_loss(mask_logits, proposals), 'vj_penalty': vj_penalty}
            else:
