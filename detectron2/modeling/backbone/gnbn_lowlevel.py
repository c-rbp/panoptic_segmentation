...
class ResNet(Backbone):
    def __init__(
        ...
        for i, blocks in enumerate(stages):
            ...
            if i in self.h_layers:
                ...
                self.add_module("posthorizontalGN{}".format(i + 2), self.horizontal_norms[horizontal_name])

        for row in self.td_groups:
            high, low = row
            layer, low_name = self.stages_and_names[low]
            if hasattr(layer, 'hidden_size'):
                fan_in = layer.hidden_size
            else:
                fan_in = layer[-1].out_channels
            layer, high_name = self.stages_and_names[high]
            if hasattr(layer, 'hidden_size'):
                td_fan_in = layer.hidden_size
            else:
                td_fan_in = layer[-1].out_channels
            self.feedback_connections += [[high_name, low_name]]
            ...
            recurrent = tdConvGRUCell(
                ...
                norm=self.rec_norm,
                grad_method=self.grad_method)
            td_name = "topdown{}".format(low_name)
            self.add_module(td_name, recurrent)
            self.stages_and_names.append((recurrent, td_name))
            self._out_feature_strides[
                td_name] = self._out_feature_strides[low_name]
            self._out_feature_channels[
                td_name] = self._out_feature_channels[low_name]
    ...
    def bptt(self, x, outputs, hidden_states, ff_activities):
        ...
        for n in range(self.timesteps):
            new_x, outputs, hidden_states, ff_activities, _ = self.inner_loop(
                x=x,
                outputs=outputs,
                hidden_states=hidden_states,
                ff_activities=ff_activities,
                n=n)
        return new_x, outputs, hidden_states, ff_activities

    def neumann(self, x, outputs, hidden_states, ff_activities, eps=0.1):
        ...
        for n in range(timesteps - 1):
            ...
            if not self.training and n == 0:
                old_x = new_x
            elif not self.training and n > self.timesteps:
                diff = torch.abs(old_x - new_x).mean()
                # print(n, diff)
                old_x = new_x
                if diff < eps:
                    break

        new_x, outputs, hidden_states, ff_activities, penalty = self.inner_loop(
            x=x,
            outputs=outputs,
            hidden_states=hidden_states,
            ff_activities=ff_activities,
            n=n+1,
            fixed_point_optim=True)
        return new_x, outputs, hidden_states, ff_activities, penalty

    def inner_loop(self, x, outputs, hidden_states, ff_activities, n, fixed_point_optim=False):
        """Run one step of inner loop."""
        td_count = 0
        penalty = 0.
        for stage, name in self.stages_and_names[1:]:
            if 'hConvGRUCell' in str(type(stage)):
                if n == 0:
                    ...
                    # hidden_states[name] = x.clone()  # torch.zeros_like(x)
                    hidden_states[name] = F.softplus(torch.zeros_like(x)).requires_grad_()
                x = stage(
                    input_=F.softplus(x),  # Changed all act funs to softplus
                    h_= hidden_states[name])
                hidden_states[name] = x

                # Add a norm and a relu
                if n == 0:  # or fixed_point_optim:  # CHECK THAT THIS IS CORRECT
                ...
                if name in self._out_features:
                    outputs[name] = x
            elif 'tdConvGRUCell' in str(type(stage)):
                feedback_map = self.feedback_connections[td_count]
                td_count += 1
                layer_id = int(name[-1])
                lower_name = 'horizontal{}'.format(layer_id)
                lower_activity = hidden_states[lower_name]
                higher_name = feedback_map[0]
                if 'res' in feedback_map[0]:
                    higher_activity = ff_activities[higher_name]
                else:
                    higher_activity = hidden_states[higher_name]
                if fixed_point_optim:
                    prev_state = lower_activity.clone().detach().requires_grad_()
                    last_state = stage(
                        lower_=prev_state,
                        higher_=higher_activity)
                    update_activity = RBPFun.apply(
                        prev_state,
                        last_state,
                        0,
                        0,
                    ...
                    update_activity = stage(
                        lower_=lower_activity,
                        higher_=higher_activity)
                hidden_states[lower_name] = update_activity
                if name in self._out_features:
...
def build_resnet_gnbn_lowlevel_model_backbone(cfg, input_shape, grad_method='bptt', timesteps=1, gala=False):
    """
    ...
    out_stage_idx = [{"topdownhorizontal2": 2, "topdownhorizontal3": 3, "topdownhorizontal4": 4, "res5": 5}[f] for f in out_features]
