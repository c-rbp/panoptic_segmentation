...
class ResNet(Backbone):
    def __init__(
        ...
        for i, blocks in enumerate(stages):
            ...
            if i in self.h_layers:
                ...
                if name == 'res5' and self.apply_output_norms is not None:
        ...
        for row in self.td_groups:
            ...
            if self.apply_output_norms is not None:
                output_norm = get_norm(self.apply_output_norms, fan_in)
                self.output_norms[td_name] = output_norm
                self.add_module("resnetoutnorm{}".format(td_name), self.output_norms[td_name])
    ...
    def forward(self, x):
        ...
        if self.apply_output_norms is not None:
            for k in outputs.keys():
                ...
                outputs[k] = F.relu_(self.output_norms[k](outputs[k]))
    ...
    def neumann(self, x, outputs, hidden_states, ff_activities, eps=1e-3):
        ...
        for n in range(timesteps - 1):
            with torch.no_grad():
                new_x, outputs, hidden_states, ff_activities, _ = self.inner_loop(  # noqa
                    x=x,
                    outputs=outputs,
                    hidden_states=hidden_states,
                    ff_activities=ff_activities,
            ...
            if not self.training and n == 0:
                old_x = new_x
            elif not self.training and n > 0:
        ...
        new_x, outputs, hidden_states, ff_activities, penalty = self.inner_loop(  # noqa
            x=x,
            outputs=outputs,
            hidden_states=hidden_states,
            ff_activities=ff_activities,
        ...
        for stage, name in self.stages_and_names[1:]:
            if 'hConvGRUCell' in str(type(stage)):
                if n == 0:
                    ...
                    # hidden_states[name] = torch.zeros_like(x).requires_grad_()
                ...
                if fixed_point_optim:
                    prev_state = lower_activity.clone().detach().requires_grad_()  # noqa
                    last_state = stage(
                        lower_=prev_state,
            ...
            # print(name, x.max(), x.min())
        return x, outputs, hidden_states, ff_activities, penalty


@BACKBONE_REGISTRY.register()
def build_resnet_gnbn_lowlevel_lesssp_backbone(cfg, input_shape, grad_method='bptt', timesteps=1, gala=False):
    """
    ...
    out_stage_idx = [{"topdownhorizontal2": 2, "topdownhorizontal3": 3, "topdownhorizontal4": 4, "horizontal5": 5}[f] for f in out_features]
