...
from .colormap import random_color, det_random_color
...
class Visualizer:
    def __init__(self, img_rgb, metadata, scale=1.0, instance_mode=ColorMode.IMAGE):
        ...
        self._default_font_size = max(
            np.sqrt(self.output.height * self.output.width) // 90, 16 // scale  # Changed 10->14
        )
    ...
    def draw_panoptic_seg_predictions(
        self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7, freeze_colors=True
    ):
        """
    ...
    def draw_dataset_dict(self, dic):
        ...
        if sem_seg is not None:
            self.draw_sem_seg(sem_seg, area_threshold=0, alpha=0.3)
        return self.output
